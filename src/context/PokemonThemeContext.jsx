  ...
  const updateActiveColors = (typeColors) => {
    if (typeColors && typeColors.bg) {
      ...
      setActivePokemonColors({
        primary,
        secondary,
        gradient: typeColors.gradient || `linear-gradient(135deg, ${primary} 0%, ${secondary} 100%)`,
      });
    } else {
      // Valor por defecto
      setActivePokemonColors({
...
export function usePokemonTheme() {
  ...
  if (!context) {
    throw new Error('usePokemonTheme debe usarse dentro de un PokemonThemeProvider');
  }
  return context;
}
